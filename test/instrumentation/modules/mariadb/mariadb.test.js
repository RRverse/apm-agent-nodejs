      ...
      t.test(executor, function (t) {
        ...
        if (hasPromises) {
          ...
          t.test('promise', function (t) {
            console.log("Probando promesas 2");
...
function assertSpan(t, span, sql) {
  console.log(span.context.destination);
  t.strictEqual(span.name, 'SELECT')
  t.strictEqual(span.type, 'db')
  t.strictEqual(span.subtype, 'mariadb')
  t.strictEqual(span.action, 'query')
  t.deepEqual(span.context.db, { statement: sql, type: 'sql' })
  t.deepEqual(span.context.destination, { service: { name: 'mariadb', resource: 'mariadb', type: 'db' }, port: 3306, address: connectionOptions.host })
}
...
function createConnection(cb) {
  setup(function () {
    ...
    queryable.connect(cb)

    mariadbPromise.createConnection(connectionOptions).then(connection => {
